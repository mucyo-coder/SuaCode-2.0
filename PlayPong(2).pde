...
float paddleHeight = 120; //var for height of the paddles
...
float leftScoreY;//sets the y position of the left player score
...
int secondLineX;
int secondLineY ; 
String prayerOne = "(mucyo AI):";
String prayerTwo = "(You):";
float prayerOneX ; 
int prayerOneY;
float prayerTwoX;
int prayerTwoY;
...
void setup() 
{
  ...
  ballY= height/2;
  leftScoreX = ballX-100;
  rightScoreX = ballX +263;
  lineX = width/2;
  secondLineX = width/2;
  secondLineY = height;
  prayerOneX = leftScoreX-120;
...
void initScreen() 
{
  ...
  textSize(30);
  text("Made by Mucyo Moses to show that no One can win AI", width/2, height/1.5);
...
void movePaddles(){
  ...
  if(ballX < width/2){
    leftPaddleY = constrain(ballY, startPoint,height - paddleHeight); //constrain paddle to drawing window
